...
const linkCls = ({ isActive }) =>
  `px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
    isActive
      ...
      : 'text-slate-600 dark:text-slate-400 hover:text-primary-600 dark:text-slate-300 dark:hover:text-primary-400'
  }`;
...
export default function Navbar({ theme = 'light', setTheme = () => {} }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-primary-100 dark:border-primary-900/50 bg-white/80 dark:bg-slate-900 dark:bg-slate-950/80 backdrop-blur-md transition-colors">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
        <Link to="/" className="font-display text-xl font-semibold tracking-tight text-slate-900 dark:text-white dark:text-white">
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Main">
          ...
          <div className="relative group">
            ...
            <div className="invisible absolute left-0 top-full z-30 mt-1 w-72 whitespace-nowrap rounded-lg border border-primary-200 dark:border-primary-800 bg-white dark:bg-slate-900 dark:bg-slate-900 p-4 opacity-0 shadow-lg transition-all duration-200 group-hover:visible group-hover:opacity-100">
              <ul className="space-y-2 text-sm text-slate-700 dark:text-slate-300 dark:text-slate-200">
          ...
          <div className="relative group">
            ...
            <div className="invisible absolute left-0 top-full z-30 mt-1 w-80 whitespace-nowrap rounded-lg border border-primary-200 dark:border-primary-800 bg-white dark:bg-slate-900 dark:bg-slate-900 p-4 opacity-0 shadow-lg transition-all duration-200 group-hover:visible group-hover:opacity-100">
              <p className="text-xs font-semibold uppercase tracking-widest text-primary-500 dark:text-primary-400">Edit & Read</p>
              <ul className="mt-2 space-y-1 text-sm text-slate-700 dark:text-slate-300 dark:text-slate-200">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-primary-300 dark:border-primary-700 px-3 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 dark:text-slate-300 hover:bg-primary-50 dark:hover:bg-primary-900/30 md:px-4 transition-colors"
          >
          ...
          {user ? (
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-slate-600 dark:text-slate-400 dark:text-slate-300 hidden sm:inline">
                {user.displayName}
              </span>
              <button
                onClick={logout}
                className="rounded-xl px-3 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 dark:text-slate-300 hover:bg-primary-50 dark:hover:bg-primary-900/30 md:px-4 transition-colors"
              >
                Log out
              </button>
            </div>
