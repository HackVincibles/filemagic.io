...
const ShaderCanvas = () => {
  ...
  useEffect(() => {
    ...
    const updateColor = () => {
      ...
      setBackgroundColor(isDark ? [0.05, 0.05, 0.05] : [0.98, 0.98, 1.0]);
    };
    updateColor();
  ...
  return <canvas ref={canvasRef} className="fixed top-0 left-0 w-full h-full block z-0" style={{ backgroundColor: 'var(--background)' }} />;
};
...
export const PricingCard = ({
  ...
  const cardClasses = \`
    ...
    dark:from-white/10 dark:to-white/5 dark:border-white/10 dark:backdrop-brightness-[0.91] z-10
  ...
  const buttonClasses = \`
    ...
    \${buttonVariant === 'primary' 
      ...
      : 'bg-black/10 hover:bg-black/20 text-slate-900 border border-black/20 dark:bg-white/10 dark:hover:bg-white/20 dark:text-white dark:border-white/20'
    }
  \`;
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-cyan-400 text-slate-900 dark:text-black z-20">
          Most Popular
        </div>
      )}
      <div className="mb-3">
        <h2 className="text-[48px] font-extralight tracking-[-0.03em] text-slate-900 dark:text-white font-display">{planName}</h2>
        <p className="text-[16px] text-slate-700 dark:text-white/70 mt-1 font-sans">{description}</p>
      </div>
      <div className="my-6 flex items-baseline gap-2">
        <span className="text-[48px] font-extralight text-slate-900 dark:text-white font-display">\${price}</span>
        <span className="text-[14px] text-slate-700 dark:text-white/70 font-sans">/mo</span>
      </div>
      ...
      <ul className="flex flex-col gap-2 text-[14px] text-slate-800 dark:text-white/90 mb-6 font-sans">
...
export const ModernPricingPage = ({
  ...
  return (
    <div className="bg-transparent min-h-screen w-full overflow-x-hidden relative">
      ...
      <main className="relative z-10 w-full min-h-screen flex flex-col items-center justify-center px-4 py-20">
        <div className="w-full max-w-5xl mx-auto text-center mb-14">
          ...
          <p className="mt-3 text-[16px] md:text-[20px] text-slate-700 dark:text-white/80 max-w-2xl mx-auto font-sans">
            {subtitle}
          </p>
        </div>
