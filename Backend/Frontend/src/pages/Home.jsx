...
import { ClientsDemo } from '../components/ui/clients.jsx';
import {
...
export default function Home() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-mist-50 via-white to-accent-50 dark:from-slate-950 dark:via-slate-900 dark:to-purple-950 transition-colors">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-20 md:py-28">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="text-center lg:text-left">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-900 dark:text-white dark:text-white mb-6">
              ...
              <p className="text-xl md:text-2xl text-slate-600 dark:text-slate-400 dark:text-slate-300 mb-6 max-w-xl mx-auto lg:mx-0">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mb-8">
                ...
                <Link
                  to="/pricing"
                  className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-2xl border-2 border-primary-200 dark:border-primary-800 bg-white dark:bg-slate-900 dark:bg-slate-900 text-slate-700 dark:text-slate-300 dark:text-slate-200 font-semibold text-lg hover:border-primary-500 hover:text-primary-600 dark:hover:text-primary-400 transition-all"
                >
                  View Pricing
                </Link>
              </div>

              <div className="flex items-center justify-center lg:justify-start gap-6 text-sm text-slate-600 dark:text-slate-400 dark:text-slate-400">
            ...
            <div className="relative">
              ...
              <div className="relative bg-white/80 dark:bg-slate-900 dark:bg-slate-900/80 backdrop-blur-md rounded-3xl p-4 shadow-2xl border border-primary-100 dark:border-primary-900">
                <FileHero3D />
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tools.map((tool, i) => (
              <Link
                key={i}
                to={tool.href}
                className="group relative bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-2xl p-6 border border-primary-100 dark:border-primary-900 hover:border-primary-300 dark:hover:border-primary-800 hover:shadow-xl hover:-translate-y-2 transition-all duration-300"
              >
                ...
                <h3 className="text-xl font-semibold text-slate-900 dark:text-white dark:text-white mb-2 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                  {tool.title}
                </h3>
      ...
      {/* Features Section */}
      <section className="py-20 md:py-28 bg-white dark:bg-slate-900 dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white dark:text-white mb-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, i) => (
              <div
                key={i}
                className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-2xl p-6 border border-primary-100 dark:border-primary-900 hover:border-primary-200 dark:hover:border-primary-800 hover:shadow-lg transition-all duration-300"
              >
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {reviews.map((review, i) => (
              <div key={i} className="bg-white dark:bg-slate-900 dark:bg-slate-900 rounded-2xl p-6 border border-primary-100 dark:border-primary-900 shadow-lg">
                ...
                <p className="text-slate-700 dark:text-slate-300 dark:text-slate-300 mb-6 italic">
                  "{review.text}"
                </p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-slate-900 dark:text-white dark:text-white">{review.name}</p>
                    <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400">{review.role}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="max-w-4xl mx-auto px-4 text-center">
          <div className="bg-gradient-to-r from-primary-600 to-accent-600 rounded-3xl p-10 md:p-16 shadow-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                to="/upload"
                className="px-8 py-4 rounded-2xl bg-white dark:bg-slate-900 text-primary-600 font-semibold text-lg hover:bg-slate-50 dark:bg-slate-800 transition-colors shadow-lg"
              >
                Start Compressing
              </Link>
              <Link
                to="/register"
                className="px-8 py-4 rounded-2xl bg-white/10 dark:bg-slate-900 border-2 border-white/50 text-white font-semibold text-lg hover:bg-white/20 dark:bg-slate-900 transition-colors"
              >
                Create Account
              </Link>
            </div>
