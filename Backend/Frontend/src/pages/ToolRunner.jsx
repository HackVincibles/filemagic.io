...
const toolConfig = {
  ...
  'pdf-to-excel': { label: 'PDF to Excel', operation: 'PDF_TO_EXCEL', accept: '.pdf' },
  'pdf-to-png': { label: 'PDF to PNG', operation: 'PDF_TO_PNG', accept: '.pdf' },
  ...
  return (
    <div className="bg-gradient-to-b from-mist-50 to-white pb-20 pt-10">
      <div className="mx-auto max-w-4xl px-4">
        <button className="text-sm text-sage-600 dark:text-sage-400 underline" onClick={() => navigate(-1)}>
          ← Back
        </button>

        <h1 className="mt-4 text-3xl font-bold text-slate-900 dark:text-white">{tool.label}</h1>
        
        <div className="mt-6 rounded-2xl border border-dashed border-sage-300/70 bg-white dark:bg-slate-900 p-6 shadow-sm">
          {!processed && (
            <>
              <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-200 dark:border-slate-700 p-10 text-center hover:border-sage-400 transition-colors">
                <div className="rounded-full bg-sage-100 p-3 text-sage-600 dark:text-sage-400 mb-3">
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                  </svg>
                </div>
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Click to upload or drag and drop</span>
                <span className="text-xs text-slate-500 dark:text-slate-400 mt-1">Accepted: {tool.accept || '*/*'}</span>
                <input
                  type="file"
                  accept={tool.accept}
                  disabled={busy}
          ...
          {processed && result && (
            <div className="mt-4 space-y-6">
              <div className="p-6 bg-sage-50 rounded-2xl border border-sage-100 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">Task Completed!</h3>
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div className="p-3 bg-white dark:bg-slate-900 rounded-xl shadow-sm">
                    <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">Original</p>
                    <p className="text-lg font-bold text-slate-700 dark:text-slate-300">{formatBytes(result.originalSize)}</p>
                  </div>
                  <div className="p-3 bg-white dark:bg-slate-900 rounded-xl shadow-sm border-2 border-sage-200">
                    <p className="text-xs text-sage-600 dark:text-sage-400 uppercase font-bold tracking-wider">Compressed</p>
              ...
              <div className="flex gap-4">
                <button 
                  className="flex-1 rounded-xl border-2 border-sage-600 py-3 text-sm font-bold text-sage-600 dark:text-sage-400 hover:bg-sage-50 transition-colors"
              ...
              <button 
                className="w-full text-xs text-slate-400 hover:text-slate-600 dark:text-slate-400 transition-colors"
                onClick={() => {
                  setProcessed(false);
                  setFile(null);
                  setResult(null);
              ...
              {previewVisible && (
                <div className="mt-6 border-t pt-6">
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-3">File Preview:</p>
                  <div className="rounded-xl overflow-hidden border bg-slate-100 dark:bg-slate-800 min-h-[200px] flex items-center justify-center">
                    {result.filename.toLowerCase().match(/\.(jpg|jpeg|png|gif|webp)$/) ? (
                      ...
                      <iframe title="Text Preview" src={objectUrl} className="h-[400px] w-full bg-white dark:bg-slate-900 p-4 font-mono text-xs" />
                      ...
                      <div className="text-center p-10">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">Direct preview not available for this file type.</p>
