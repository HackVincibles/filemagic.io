...
export default function FileUpload({ showAdPlaceholder = true, initialOperation = 'COMPRESS' }) {
  ...
  return (
    <div className="fm-glass rounded-2xl p-6 shadow-soft md:p-8">
      {showAdPlaceholder && (
        <div className="mb-6 rounded-xl border border-dashed border-slate-200 bg-mist-50/80 px-4 py-3 text-center text-xs text-slate-500">
      ...
      <div
        ...
        onDrop={onDrop}
        className="fm-card-3d cursor-pointer rounded-2xl border-2 border-dashed border-sage-300/60 bg-white/50 px-6 py-10 text-center"
      >
        <input
          type="file"
          className="sr-only"
        ...
        <label htmlFor={fileInputId} className="cursor-pointer">
          <p className="font-display text-lg font-semibold text-slate-800">Drop a file here</p>
          <p className="mt-2 text-sm text-slate-500">or click to browse — one file at a time</p>
          {plan && (
        ...
        {file && (
          <p className="mt-4 text-sm font-medium text-sage-600">
          ...
          <button
            ...
            className={`rounded-xl border px-4 py-3 text-left transition-all ${
              operation === op.id
                ...
                : 'border-slate-200 bg-white/80 hover:border-sage-300'
            }`}
          >
            <p className="font-semibold text-slate-800">{op.label}</p>
            <p className="mt-1 text-xs text-slate-500">{op.hint}</p>
          </button>
        ))}
      </fieldset>

      {operation === 'COMPRESS' && (
        <p className="mt-4 rounded-lg border border-slate-200 bg-mist-50 px-4 py-3 text-sm text-slate-600">
      ...
      {operation === 'CONVERT' && (
        <label className="mt-4 block text-sm font-medium text-slate-700" htmlFor={convertSelectId}>
          Conversion mode
          <select
            id={convertSelectId}
      ...
      {error && (
        <div className="mt-3 text-center">
          ...
          {error.includes('limit reached') && (
            <Link to="/pricing" className="mt-2 inline-block text-xs font-semibold text-sage-600 underline">
