  ...
  return (
    <div className="bg-gradient-to-b from-mist-50 to-white pb-20 pt-10">
      <div className="mx-auto max-w-6xl px-4">
        <h1 className="font-display text-3xl font-bold text-slate-900">PDF Tools</h1>
        <p className="mt-2 text-slate-600">All PDF operations in one place, with a workflow that redirects to a dedicated upload process.</p>

        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          {categories.map((category) => (
            <section key={category.title} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-3 grid gap-2">
                {category.items.map((item) => (
                  <Link
                    ...
                    className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:border-sage-300 hover:bg-sage-50"
                  >
                    {item}
                  </Link>
                ))}
