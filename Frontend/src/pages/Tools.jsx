  ...
  return (
    <div className="bg-gradient-to-b from-mist-50 to-white pb-20 pt-10">
      <div className="mx-auto max-w-6xl px-4">
        <h1 className="font-display text-3xl font-bold text-slate-900">All PDF tools</h1>
        <p className="mt-2 text-slate-600">Pick a tool and process directly with a dedicated drop zone, preview, and download.</p>

        {tools.map((section) => (
          <section key={section.category} className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-xl font-semibold text-slate-800">{section.category}</h2>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {section.items.map((item) => (
                <Link
                  key={item}
                  to={`/tool/${toRouteId(item)}`}
                  className="rounded-xl border border-slate-200 p-3 text-sm font-medium text-slate-700 hover:border-sage-300 hover:bg-sage-50"
                >
                  {item}
                </Link>
              ))}
